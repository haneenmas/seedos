...
#include "disasm.hpp"
#include "trace.hpp"
#include "elf.hpp"      // we try to load ELF; otherwise fallback to demos
...
// ---------- tiny helpers: write words + encode a few RV32I ops ----------
static inline void put32(Memory& m, uint32_t addr, uint32_t word){
    m.store32(addr, word);
}

// I-type encoder (ADDI etc.), opcode 0x13
static inline uint32_t enc_I(uint8_t rd, uint8_t rs1, int32_t imm12, uint8_t funct3){
    uint32_t u = (uint32_t)(imm12 & 0xFFF);
    return (u << 20) | (rs1 << 15) | (funct3 << 12) | (rd << 7) | 0x13;
}
...
static void load_task_program(Memory& ram, uint32_t base, int which){
    ...
    if (which == 1){
        // Task A
        put32(ram, a+0x00, enc_I(/*rd=*/1, /*rs1=*/0, /*imm=*/5,  0));      // addi x1,x0,5
...
// =========================== main ===========================
int main(){
    // 0) Bring up memory + a CPU we can reuse
    Memory ram(64*1024);
    CPU cpu; cpu.pc = 0;
    ...
    std::cout << "[user] demo: write string via ECALL\n";
    {
        ...
        um.store32(0x00, enc_LUI(10, BASE>>12));                     // a0 = upper
        um.store32(0x04, enc_I(10,10,(int32_t)(BASE & 0xFFF),0));    // a0 += lower
        um.store32(0x08, enc_I(11, 0, (int32_t)len, 0));             // a1 = len
        um.store32(0x0C, enc_I(17, 0, 5, 0));                        // a7 = write
        um.store32(0x10, encSYSTEM(0));                              // ecall
        um.store32(0x14, enc_I(10, 0, 0, 0));                        // a0=0
        um.store32(0x18, enc_I(17, 0, 0, 0));                        // a7=0 (exit)
        um.store32(0x1C, encSYSTEM(0));                              // ecall(exit)
    ...
    std::cout << "\n[dbg] breakpoints + single-step demo\n";
    {
        ...
        m.store32(0x00, enc_I(1,0,5,0));                 // addi x1,x0,5
        m.store32(0x04, enc_I(2,1,10,0));                // addi x2,x1,10
        m.store32(0x08, enc_R(4,1,2,0,0));               // add x4,x1,x2
        m.store32(0x0C, enc_R(5,4,1,0,0x20));            // sub x5,x4,x1
    ...
    run_round_robin_preemptive_demo();
    return 0;
}
